 ...
 * @returns 插件上下文
 */
function createPluginContext(optionsRef: {
  value: MapDxfExportOptions;
...
describe("mapDxfExportPlugin", () => {
  ...
  it("getResolvedOptions 应返回已合并的 TrueColor 解析器字段", () => {
    ...
    expect(defaultOptions.layerTrueColorResolver).toBe(
      pageLayerTrueColorResolver,
    );
    expect(defaultOptions.featureTrueColorResolver).toBe(
      pageFeatureTrueColorResolver,
    );
    expect(overrideOptions.layerTrueColorResolver).toBe(
      overrideLayerTrueColorResolver,
    );
    expect(overrideOptions.featureTrueColorResolver).toBe(
      pageFeatureTrueColorResolver,
    );
  });
