 ...
 * @param mapExpose 地图公开实例
 * @param pluginId 历史兼容参数；传入时会额外校验唯一插件 ID
 * @returns 命中的面边线预览插件 API；找不到时返回 null
 */
export function resolvePolygonEdgePreviewApi(
  mapExpose: MapLibreInitExpose | null | undefined,
  pluginId?: string
): PolygonEdgePreviewPluginApi | null {
  const pluginTarget = resolveMapPluginTargetByType(
    mapExpose,
  ...
  return mapExpose?.plugins.getApi<PolygonEdgePreviewPluginApi>(pluginTarget.id) || null;
}
 ...
 * @param mapExpose 地图公开实例
 * @param pluginId 历史兼容参数；传入时会额外校验唯一插件 ID
 * @returns 命中的面边线预览插件状态；找不到时返回 null
 */
export function resolvePolygonEdgePreviewState(
  mapExpose: MapLibreInitExpose | null | undefined,
  pluginId?: string
): PolygonEdgePreviewState | null {
  const pluginTarget = resolveMapPluginTargetByType(
    mapExpose,
    POLYGON_EDGE_PREVIEW_PLUGIN_TYPE,
    pluginId
  ...
  return mapExpose?.plugins.getState<PolygonEdgePreviewState>(pluginTarget.id) || null;
}
