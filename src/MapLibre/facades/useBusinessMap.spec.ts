...
function createLineDraftPluginHarness(): {
  ...
  const api = {
    data: ref(
      createFeatureCollection([
        createPointFeature("draft-1", { name: "草稿" }),
      ]),
    ),
    lineStyle: ref(null),
    fillStyle: ref(null),
    getFeatureById: (featureId: MapFeatureId | null) => {
      return featureId === "draft-1"
        ? createPointFeature("draft-1", { name: "草稿" })
        : null;
    },
...
function createMapExpose(
  ...
  const pluginHost: MapPluginHostExpose = {
    ...
    getState: <TState = unknown>(pluginId?: string) => {
      ...
      if (pluginId === "mapFeatureSnap") {
        return (
          (((snapState as any)?.value ?? snapState) as TState | null) || null
        );
      }
...
describe("useBusinessMap", () => {
  it("business 和 root 入口不公开插件第二套读取门面", () => {
    const businessEntrySource = readFileSync(
      resolve(__dirname, "../../entries/business.ts"),
      "utf-8",
    );
    const rootEntrySource = readFileSync(
      resolve(__dirname, "../../entries/root.ts"),
      "utf-8",
    );
  ...
  it("business 入口应直接导出业务层高频类型", () => {
    ...
    expect(businessEntrySource).toContain(
      "export type { MapBusinessSourceRegistry }",
    );
    expect(businessEntrySource).toContain(
      "export type { MapBusinessLayerDescriptor }",
    );
    expect(businessEntrySource).toContain(
      "export type { MapFeaturePropertyPanelItem }",
    );
    expect(businessEntrySource).toContain(
      "export type { MapSourceFeatureRef }",
    );
    expect(businessEntrySource).toContain("export type { MapFeatureId }");
    expect(businessEntrySource).toContain(
      "export type { UseBusinessMapPlugins }",
    );
  });

  it("plugins 聚合入口应导出业务插件预设工厂和全部插件子入口", () => {
    const pluginsEntrySource = readFileSync(
      resolve(__dirname, "../../entries/plugins.ts"),
      "utf-8",
    );

    expect(pluginsEntrySource).toContain("export { createBusinessPlugins }");
    expect(pluginsEntrySource).toContain(
  ...
  it("plugins 聚合入口应同步 package、Vite、Vitest 和 TypeScript 路径", () => {
    ...
    expect(viteConfigSource).toContain(
      'plugins: fileURLToPath(new URL("./src/plugins.ts"',
    );
    ...
    expect(tsConfigSource).toContain(
      '"vue-maplibre-kit/plugins": ["./src/plugins.ts"]',
    );
  });
  ...
  it("会按分组暴露业务 source、查询与动作能力", () => {
    ...
    const feature = businessMap.feature.resolveFeature(
      source.toFeatureRef("feature-1"),
    );
  ...
  it("会复用选择态、线草稿和特效分组", () => {
    ...
    expect(businessMap.selection.selectedFeatureIds.value).toEqual([
      "feature-1",
    ]);
    ...
    expect(businessMap.plugins.dxfExport.isExporting.value).toBe(false);
    expect(businessMap.plugins.dxfExport.getResolvedOptions()?.fileName).toBe(
      "business.dxf",
    );
    expect(flashStartResult).toBe(true);
    expect(
  ...
  it("会暴露运行时图层快捷动作", () => {
    ...
    const rawMap = {
      getLayer: vi.fn((layerId: string) =>
        layerId === "line-layer" ? { id: layerId } : null,
      ),
    ...
    const featureStateResult = businessMap.layers.setFeatureState(
      "business-source",
      "feature-1",
      {
        demoStyled: true,
    ...
    expect(rawMap.setPaintProperty).toHaveBeenCalledWith(
      "line-layer",
      "line-color",
      "#f97316",
    );
