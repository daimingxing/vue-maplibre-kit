...
describe("createMapBusinessSource", () => {
  it("异步业务源在 null 或 undefined 首屏时会自动兜底为空 FeatureCollection，并允许后续回填", async () => {
    ...
    expect(
      toRaw(source.sourceProps.data as MapCommonFeatureCollection),
    ).toEqual({
      type: "FeatureCollection",
      features: [],
    });
    ...
    expect(
      (source.sourceProps.data as MapCommonFeatureCollection).features,
    ).toHaveLength(1);
    expect(source.resolveFeature("feature-async")?.properties?.id).toBe(
      "feature-async",
    );
  });
  ...
  it("promoteId 路径初始化时只复制 features 数组，不深拷贝整条 feature", () => {
    ...
    expect(toRaw(normalizedCollection.features?.[0] as MapCommonFeature)).toBe(
      originalFeature,
    );
  ...
  it("featureIdKey 路径只复制需要补顶层 id 的 feature", () => {
    ...
    const data = ref(
      createFeatureCollection([featureNeedSync, featureAlreadySynced]),
    );
    ...
    expect(
      toRaw(normalizedCollection.features?.[0].properties as FeatureProperties),
    ).toBe(featureNeedSync.properties);
    expect(toRaw(normalizedCollection.features?.[1] as MapCommonFeature)).toBe(
      featureAlreadySynced,
    );
  });
  ...
  it("getFeatureId 路径本地属性写回后仍会回退到全量标准化", () => {
    ...
    const data = ref(
      createFeatureCollection([
        createPointFeature("feature-1", {
          properties: { bizId: "feature-1", status: "old" },
        }),
        createPointFeature("feature-2", {
          properties: { bizId: "feature-2", status: "keep" },
        }),
      ]),
    );
    const source = createMapBusinessSource({
    ...
    const sentinelCollection = createFeatureCollection([
      createPointFeature("feature-1", {
        properties: { bizId: "feature-1", status: "new" },
      }),
      createPointFeature("feature-2", {
        properties: { bizId: "feature-2", status: "keep" },
      }),
  ...
  it("动态注册表应支持空初始化后 addSource 并保持 listSources 可响应追踪", async () => {
    const registry = createMapBusinessSourceRegistry();
    const source = createMapBusinessSource({
      sourceId: "business-dynamic",
      data: ref(createFeatureCollection([createPointFeature("feature-1")])),
      promoteId: "id",
    });
    const sourceCount = computed(() => registry.listSources().length);
    ...
    expect(registry.getSource("business-dynamic")).toBe(source);
    expect(registry.listSources()).toEqual([source]);
    expect(sourceCount.value).toBe(1);
  });

  it("动态注册表应支持 setSources 批量替换", () => {
    const registry = createMapBusinessSourceRegistry();
    const primarySource = createMapBusinessSource({
      sourceId: "business-primary",
      data: ref(createFeatureCollection([createPointFeature("feature-1")])),
      promoteId: "id",
    });
    const nextSource = createMapBusinessSource({
      sourceId: "business-next",
      data: ref(createFeatureCollection([createPointFeature("feature-2")])),
      promoteId: "id",
    });

    registry.addSource(primarySource);
    registry.setSources([nextSource]);

    expect(registry.getSource("business-primary")).toBeNull();
    expect(registry.getSource("business-next")).toBe(nextSource);
    expect(registry.listSources()).toEqual([nextSource]);
  });

  it("动态注册表应支持 removeSource 与 clearSources", () => {
    const registry = createMapBusinessSourceRegistry();
    const primarySource = createMapBusinessSource({
      sourceId: "business-primary",
      data: ref(createFeatureCollection([createPointFeature("feature-1")])),
    ...
    registry.setSources([primarySource, nextSource]);
    registry.removeSource("business-primary");
    ...
    expect(registry.getSource("business-next")).toBeNull();
    expect(registry.listSources()).toEqual([]);
  });

  it("动态注册表遇到重复 sourceId 时会直接抛出异常", () => {
    const registry = createMapBusinessSourceRegistry();
    const primarySource = createMapBusinessSource({
  ...
  it("layers 支持 ref 与 getter 输入，并在非法值时回退为空数组", () => {
    ...
    const layerRef = ref(layerList) as unknown as {
      value: MapBusinessLayerDescriptor[];
    };
  ...
  it("source 默认 propertyPolicy 会被 layer 继承，并允许按字段局部覆写", () => {
    ...
    const layerPanel = source.resolvePropertyPanelState(
      "feature-1",
      "circle-layer",
    );

    expect(
      sourcePanel?.items.find((item) => item.key === "name"),
    ).toMatchObject({
      editable: false,
      removable: false,
      temporary: false,
    ...
    expect(source.resolveFeature("feature-1")?.properties?.name).toBe(
      "原始名称",
    );
    ...
    const removeResult = source.removeProperties(
      "feature-1",
      ["status"],
      "circle-layer",
    );
    ...
    expect(source.resolveFeature("feature-1")?.properties?.status).toBe(
      "draft",
    );
  });
});
