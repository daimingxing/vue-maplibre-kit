...
4. 新增的能力是否在`examples/views/NG/GI/`的示例页中有所体现
5. 示例页是否改用公开出口
6. mock 数据是否留在业务模拟层
7. 生成代码后执行一次typeScript检查
8. 遇到意外情况、理解障碍、思维卡点或踩坑时，记录到 `docs/problem-record.md`。

## 架构要点
- **核心容器**：`src/MapLibre/core/mapLibre-init.vue` 管理地图、控件、绘图、插件
- **插件系统**：`defineMapPlugin()` 定义，`plugins` prop 注册，内置 snap、line-draft、intersection、multi-select、dxf-export、polygon-edge 插件
- **业务插件入口**：业务层优先用 `vue-maplibre-kit/plugins` 的 `createBusinessPlugins()` 注册常用插件，单插件子路径只保留给深度定制、常量和高级类型
- **业务插件读取**：业务层统一通过 `useBusinessMap().plugins.*` 读取插件状态与动作；不要再从业务入口公开单插件 `use*` 读取门面
...
## 知识库维护
- 业务使用说明优先写入 `docs/vue-maplibre-kit-knowledge/**`
- 定位源码和实现关系时优先看 `docs/file-index.md` 和公开入口源码，不要为了找文件先绕到知识库
- 知识库推荐路径必须保持：`vue-maplibre-kit/business` 接业务页面，`vue-maplibre-kit/plugins` 注册常用插件，`vue-maplibre-kit/config` 配全局默认值，`vue-maplibre-kit/geometry` 做几何计算
- 插件文档统一表达为：用 `createBusinessPlugins()` 注册，用 `useBusinessMap().plugins.*` 读取状态和动作
