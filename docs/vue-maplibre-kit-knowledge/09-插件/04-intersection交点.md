...
- `sourceRegistry` 传入后，插件会自动从业务 source 注册表提取候选线。
...
- `examples/views/NG/GI/NGGI06.vue`：五插件总览中的刷新交点、生成正式交点、删除正式交点。

## 风险提示

- `targetSourceIds` 是必填业务范围；缺少业务 source 或 source 中没有线要素时，刷新不会得到预期交点。
