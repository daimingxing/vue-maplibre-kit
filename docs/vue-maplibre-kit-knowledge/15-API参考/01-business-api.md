...
| `createMapBusinessSourceRegistry` | 函数 | 无参创建动态业务 source 注册表，通过 `addSource()`、`setSources()`、`removeSource()`、`clearSources()` 管理 source 集合 | 推荐     |
...
| `createLayerGroup`                | 函数 | 基于必填 `sourceId` 批量创建同一 source 下的逻辑图层，完整 layerId 为 `${sourceId}-${id}`                               | 推荐     |
