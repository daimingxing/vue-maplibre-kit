...
- **业务插件入口**：业务层优先用 `vue-maplibre-kit/plugins` 的 `createBusinessPlugins()` 注册常用插件，单插件子路径保留给高级用法
- **业务插件读取**：业务层统一通过 `useBusinessMap().plugins.*` 读取 snap、line-draft、intersection、multi-select、dxf-export 等插件状态与动作
